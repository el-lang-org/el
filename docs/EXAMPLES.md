...
source as a one-element list. The separator defaults to a single ASCII space,
so `String.split("one two")` returns `["one", "two"]`. Use
`String.graphemes` when the desired unit is a Unicode extended grapheme cluster.
